...
import com.financeflow.model.PAIDDatum
...
class PaidAdapter(private val c: Context,) :
    RecyclerView.Adapter<PaidAdapter.BeatsViewHolder>() {
    private var userlist: List<PAIDDatum>? = null
    private var userId: Int = 0
    private var promolink : String = "test";

    fun setList(userlist1: List<PAIDDatum>,) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BeatsViewHolder {
        ...
        val v = LayoutInflater.from(parent.context).inflate(R.layout.item_paidadapter, parent, false)
...
//       Glide.with(c).load("https://celebrityschool.in/images/mobile_app/singing_competition_card_image.jpg").into(holder.courseimg)
     ...
     /*   holder.mName.text = userlist!!.get(position).title!!.trim()
        ...
        holder.mNamBy.text = "Paid by "+userlist?.get(position)?.borrower!!.fullName.toString()
